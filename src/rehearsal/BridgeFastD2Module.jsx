...
/* ============================================================================
   THE 3RD ACADEMY · BridgeFast™ Engine — D2 Production Build
   Accountability & Ownership. Same engine as D1; D2-specific content.
   Aligned to Dimension Production Standard rev0 (locked, May 2026).
   Cover → A (cold-open + artifact-write) → B (B-0 reference calibration) →
   C → D (audio case, 2 decisions, artifact-write) → E (SC1–SC4) → F → G
   ========================================================================== */
...
const SCENARIOS = [SC1_CONTENT_D2, SC2_CONTENT_D2, SC3_CONTENT_D2, SC4_CONTENT_D2];
  ...
  return (
    <Stage bg={C.navyDeep} narrow>
      <div style={{ textAlign: "center", paddingTop: 30 }}>
        ...
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 2, color: "rgba(255,255,255,0.45)", marginTop: 6 }}>FOUNDATIONAL TIER · BEHAVIOURAL DIMENSION No. 2</div>
        ...
        <p style={{ fontFamily: SERIF, fontStyle: "italic", fontSize: 17, color: "rgba(255,255,255,0.7)", lineHeight: 1.5, maxWidth: 440, margin: "0 auto" }}>
          A behavioural rehearsal. Ninety minutes. Private practice. No ratings. No pass or fail.
        </p>
  ...
  return (
    <Stage bg={C.navyDeep} narrow>
      <div style={{ textAlign: "center" }}>
        ...
        <h1 style={{ fontFamily: SERIF, fontWeight: 400, fontSize: 26, color: C.white, lineHeight: 1.4, marginBottom: 10 }}>Behavioural Dimension No. 2 — Accountability &amp; Ownership</h1>
  ...
  return (
    <Stage bg={C.navyDeep} narrow>
      <div style={{ textAlign: "center", opacity: fading ? 0 : 1, transition: "opacity 0.6s ease" }}>
        ...
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 2, color: C.tealMid, marginBottom: 16 }}>THE REPAIR SEQUENCE</div>
...
/* ---- A-4 Central Question — silent 4-second hold per D2 script.
   The participant must sit with the question. No narration, no tap to advance,
   no back button. Mirrors the visual restraint of the G-4 bookend. ---- */
function A4CentralQuestionScreen({ text, onDone, holdSeconds = 4 }) {
  const [fading, setFading] = useState(false);
  useEffect(() => {
    const tFade = setTimeout(() => setFading(true), Math.max(0, (holdSeconds - 0.6) * 1000));
    const tDone = setTimeout(() => onDone(), holdSeconds * 1000);
  ...
  return (
    <Stage bg={C.navyDeep} narrow>
      <div style={{ textAlign: "center", opacity: fading ? 0 : 1, transition: "opacity 0.6s ease" }}>
        ...
        <p style={{ fontFamily: SERIF, fontSize: "clamp(20px, 4.5vw, 24px)", color: C.white, lineHeight: 1.5 }}>{text}</p>
      </div>
    </Stage>
  );
}
...
const initialState = {
  ...
  paused: false,
  coldOpenPath: null,
  coldOpenArtifactWriteText: "", // D2-specific: captured at A-1, surfaced at B-0
  scenarioPath: null,
  ledger: [],
  finalSentence: "",
  scenarioIndex: 0,
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "COLD_OPEN":
      // D2 routes to a1_artifact (cold-open artifact-write) before a2.
      return { ...state, coldOpenPath: action.path, screen: "a1_artifact", history: [...state.history, state.screen] };
    case "SET_COLD_OPEN_ARTIFACT":
      return { ...state, coldOpenArtifactWriteText: action.text };
    ...
    case "SET_D_PAUSE2":
      return { ...state, segmentDPause2: action.value };
    case "SET_D_PAUSE2_ARTIFACT":
      return { ...state, segmentDPause2ArtifactText: action.text };
    case "SET_F_ANSWER": {
...
export default function BridgeFastD2ModuleRoot() {
  return (
    <ModuleErrorBoundary>
      <PiperProvider>
        <BridgeFastD2Module />
...
function BridgeFastD2Module() {
  ...
  const C0 = D2_CONTENT.segmentA.coldOpen;
  const SC = SCENARIOS[st.scenarioIndex];
  const SG = D2_CONTENT.segmentG;
  ...
  /* ============== SEGMENT A ============== */
      ...
      <div style={{ minHeight: "100%", background: a0phase === 0 ? "#000" : C.navyDeep, display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.5s ease", padding: 20 }}>
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontFamily: SERIF, fontStyle: "italic", fontSize: 20, color: C.tealMid, lineHeight: 1.5, marginTop: 24, maxWidth: 460, opacity: a0phase >= 1 && a0phase < 2 ? 1 : 0, transition: "opacity 1s" }}>
            {D2_CONTENT.dimension.central_question}
          </div>
        </div>
      </div>
    );
    ...
    // D1 → D2 cold-open callback. Per Standard §1.4.A, the personalized
    // variant reads back the participant's D1 final personal sentence
    // verbatim. The personalized variant cannot be pre-rendered (the
    // sentence is unknown at build time) — for MVP we render the fallback
    // string per script and play it as live audio.
    // TODO(cross-module-memory): once the cross-module memory store is
    // wired up, replace the fallback with a stitched playback —
    // pre-rendered wrapper clip (parts 1+2) + Web Speech API for the
    // participant's stored sentence, per CLAUDE_PROGRESS_MEMORY.md "D1
    // → D2 cold open" implementation plan. The fallback path must
    // continue to work.
    const callbackText = D2_CONTENT.segmentA.coldOpenCallback.fallback;
    body = (
      <Stage>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 2, color: C.tealMid, marginBottom: 10 }}>SEGMENT A · COLD OPEN</div>
        {/* Beat 1 — D1 callback (fallback variant at MVP) */}
        <div style={{ marginBottom: 18, padding: "16px 18px", borderRadius: 10, border: `1px solid ${C.tealMid}`, background: "rgba(94,234,212,0.06)" }}>
          <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1.5, color: C.tealMid, fontWeight: 700, marginBottom: 8, textTransform: "uppercase" }}>D1 → D2 callback</div>
          <AVPlaceholder label="A-1 · Beat 1 — D1 callback (fallback)" text={callbackText} />
          <p style={{ fontFamily: SERIF, fontStyle: "italic", fontSize: 15.5, color: "rgba(255,255,255,0.85)", lineHeight: 1.65, margin: "8px 0 0" }}>{callbackText}</p>
        </div>
        {/* Beat 2 — Cold-open narration. The script artifacts (sprint
            board + standup countdown) render as flavour cards. */}
        <AVPlaceholder label="A-1 · Beat 2 — cold open narration" text={C0.narration.join("\n\n")} />
        <Narration lines={C0.narration} speakable={false} />
        <div style={{ display: "flex", flexDirection: "column", gap: 10, margin: "8px 0 24px" }}>
          <Artifact title="Jira · sprint board — ticket #4297 highlighted" mono>
            Ticket #4297 — Search filter (region + product type)<br />
            <span style={{ background: C.paleRed, color: C.redInk, padding: "1px 4px", borderRadius: 3 }}>Status: In Progress · committed: this sprint</span><br />
            Linked PR: #2841 (Draft · 14 commits · incomplete)
          </Artifact>
          <Artifact title="Standup countdown · 9:14 AM Thursday" mono>
            Standup begins in <span style={{ color: C.amber, fontWeight: 700 }}>15:00</span> and decreasing.
          </Artifact>
        </div>
  ...
  // D2's cold-open artifact-write — the participant writes the actual
  // Slack message / in-standup statement. Reference versions surface
  // at B-0, not here, per the script's hard-cap protection.
  else if (st.screen === "a1_artifact") {
    body = (
      <Stage>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, marginBottom: 14 }}>A-1 · COLD-OPEN ARTIFACT-WRITE</div>
        <AVPlaceholder label="A-1 · artifact-write prompt" text={C0.artifactWritePrompt} />
        <p style={{ fontFamily: SERIF, fontSize: 17, color: "rgba(255,255,255,0.9)", lineHeight: 1.65, marginBottom: 16 }}>{C0.artifactWritePrompt}</p>
        <DPause2Form persistKey={st.screen} prompt="" submit={C0.artifactWriteSubmit} min={30}
          onDone={(t) => { dispatch({ type: "SET_COLD_OPEN_ARTIFACT", text: t }); goto("a2"); }} />
      </Stage>
    );
  }
    ...
    const sf = D2_CONTENT.segmentA.safetyFloor;
    body = (
      <Stage bg={C.navyDeep} narrow>
        ...
        {sf.narration && (
          <div style={{ marginTop: 22 }}>
            <AVPlaceholder label="A-3 · safety-floor narration" text={sf.narration.join("\n\n")} />
            <Narration lines={sf.narration} speakable={false} />
          </div>
        )}
  ...
  // No back, no tap to advance. Auto-advances.
  else if (st.screen === "a4") {
    body = <A4CentralQuestionScreen text={D2_CONTENT.dimension.central_question} onDone={() => goto("b0")} />;
  }

  /* ============== SEGMENT B — REFERENCE CALIBRATION + STANDARD ============== */
  // B-0 — surfaces the cold-open artifact-write alongside the three reference
  // versions. Per script, this is the architecturally right beat: cool
  // reflection after hot pressure.
  else if (st.screen === "b0") {
    const b0 = D2_CONTENT.segmentB.b0;
    body = (
      <Stage bg={C.navyDeep}>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 2, color: C.tealMid, marginBottom: 10 }}>SEGMENT B · B-0 · REFERENCE CALIBRATION</div>
        <AVPlaceholder label={b0.label} text={b0.narration.join("\n\n")} />
        <Narration lines={b0.narration} speakable={false} />
        {/* Participant's own message — teal-bordered card per script */}
        <div style={{ marginTop: 18, background: "rgba(13,148,136,0.12)", border: `1.5px solid ${C.teal}`, borderRadius: 10, padding: 16 }}>
          <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, textTransform: "uppercase", marginBottom: 8 }}>{b0.yourMessageLabel}</div>
          <div style={{ fontFamily: SERIF, fontSize: 15.5, color: C.white, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>
            {st.coldOpenArtifactWriteText || "(no message captured — render fallback)"}
          </div>
        </div>
        <div style={{ marginTop: 22, fontFamily: SANS, fontSize: 12, letterSpacing: 0.5, color: "rgba(255,255,255,0.5)", textTransform: "uppercase" }}>Three reference versions</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 12 }}>
          {b0.references.map((r, i) => (
            <div key={i} style={{ borderRadius: 10, padding: 14, background: "rgba(255,255,255,0.04)", border: r.calibrated ? `1.5px solid ${C.tealMid}` : "1px solid rgba(255,255,255,0.1)" }}>
              <div style={{ fontFamily: SANS, fontSize: 11, fontWeight: 700, letterSpacing: 0.5, color: r.calibrated ? C.tealMid : "rgba(255,255,255,0.5)", marginBottom: 6 }}>{r.tag}</div>
        ...
        <p style={{ fontFamily: SERIF, fontStyle: "italic", fontSize: 14.5, color: "rgba(255,255,255,0.6)", marginTop: 16, textAlign: "center", lineHeight: 1.6 }}>
          {b0.closing}
        </p>
  ...
  /* ============== SEGMENT B — BEHAVIOUR STANDARD ============== */
    ...
    body = (
      <Stage bg={C.navyDeep} narrow>
        ...
        <AVPlaceholder label="B-1 introduction · 25s" text={D2_CONTENT.segmentB.intro.join("\n\n")} />
        <Narration lines={D2_CONTENT.segmentB.intro} speakable={false} />
    ...
    const fg = D2_CONTENT.segmentB.fieldGuide;
    body = (
      <Stage narrow>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 2, color: C.tealMid, marginBottom: 10 }}>BEHAVIOUR STANDARD · D2 · FOUNDATIONAL</div>
    ...
    body = (
      <Stage narrow>
        ...
        <Reflection persistKey={st.screen} prompt={D2_CONTENT.segmentB.reflectionPrompt} minChars={30} onDone={(text) => {
  ...
  /* ============== SEGMENT C — RECOGNITION BRIEFS ============== */
    ...
    const c1 = D2_CONTENT.segmentC.c1;
    body = (
      <Stage>
        ...
        <div style={{ background: "rgba(13,148,136,0.08)", border: `1px solid ${C.teal}`, borderRadius: 10, padding: 18, margin: "18px 0" }}>
          <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, marginBottom: 14 }}>THE FIRST THIRTY SECONDS — TWO CASES</div>
          {c1.cases.map((s) => (
        ...
        <AVPlaceholder label="C1 narration · part 2 — after the two cases" text={c1.close} />
    ...
    body = (
      <Stage narrow>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, marginBottom: 14 }}>C1 · BRIEF WRITTEN RESPONSE</div>
        <Reflection persistKey={st.screen} prompt={D2_CONTENT.segmentC.c1.prompt} minChars={30} onDone={(text) => {
    ...
    const c2 = D2_CONTENT.segmentC.c2;
    body = (
      <Stage>
    ...
    const cq = D2_CONTENT.segmentC.c2.centralQuestionReturn;
    ...
    // D2 C2 is a single-sentence rewrite (no multiple choice per script).
    body = (
      <Stage narrow>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, marginBottom: 14 }}>C2 · BEHAVIOUR-CENTRED REWRITE</div>
        <Reflection persistKey={st.screen} prompt={D2_CONTENT.segmentC.c2.prompt} minChars={30} onDone={(text) => {
          dispatch({ type: "SET_C_RESPONSE", key: "c2", value: text });
          goto("c3");
        }} />
      </Stage>
    );
    ...
    const c3 = D2_CONTENT.segmentC.c3;
    body = (
      <Stage bg={C.navyDeep} narrow>
        ...
        <ClickToReveal buttonLabel="Reveal the Repair Sequence">
          <div style={{ textAlign: "center", padding: "28px 16px", background: C.paper, color: C.ink, borderRadius: 12, margin: "0 0 24px" }}>
            <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 2, color: C.teal, marginBottom: 14 }}>THE REPAIR SEQUENCE</div>
    ...
    body = (
      <Stage narrow>
        ...
        <Reflection persistKey={st.screen} prompt={D2_CONTENT.segmentC.c3.prompt} minChars={30} onDone={(text) => {
  ...
  /* ============== SEGMENT C → D — CLOSE AND PAUSE INVITATION ============== */
    ...
    const cc = D2_CONTENT.segmentC.complete;
    ...
    const bp = D2_CONTENT.segmentC.breakPoint1;
    body = (
      <Stage bg={C.navyDeep} narrow>
  ...
  /* ============== SEGMENT D — AUDIO CASE ============== */
    ...
    body = (
      <Stage bg={C.navyDeep} narrow>
        ...
        <h2 style={{ fontFamily: SERIF, fontSize: 28, color: C.white, lineHeight: 1.3, marginBottom: 20 }}>{D2_CONTENT.segmentD.title}</h2>
        <AVPlaceholder label="Segment D · audio case intro" text={D2_CONTENT.segmentD.intro.join("\n\n")} />
        <Narration lines={D2_CONTENT.segmentD.intro} speakable={false} />
        <PrimaryButton onClick={() => goto("d2")}>Begin the story</PrimaryButton>
      </Stage>
    );
  }

  // D-2 — Part 1 narration + artifacts (Priya's hotfix)
  else if (st.screen === "d2") {
    const p1 = D2_CONTENT.segmentD.part1;
    body = (
      <Stage>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, marginBottom: 10 }}>PART 1 · THE DISCOVERY</div>
        <AVPlaceholder label="Audio Part 1 · The Discovery" text={p1.narration.join("\n\n")} />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, margin: "16px 0 8px" }}>
          <Artifact title={p1.slackArtifact.title}>
            <div style={{ whiteSpace: "pre-wrap" }}>{p1.slackArtifact.body}</div>
          ...
          <Artifact title={p1.timelineArtifact.title} mono>
            {p1.timelineArtifact.lines.map((l, i) => <div key={i}>{l || " "}</div>)}
          </Artifact>
        </div>
  ...
  // D-3 — Decision Pause 1 (3 paths a/b/c per script)
  else if (st.screen === "d3") {
    const p = D2_CONTENT.segmentD.pause1;
    body = (
      <Stage>
  ...
  // D-4 — Decision 1 consequence (single horizon, no bell)
  else if (st.screen === "d4") {
    const p = D2_CONTENT.segmentD.pause1;
    const chosen = st.segmentDPause1?.path;
    const lines = chosen ? p.consequences[chosen] : [];
    body = (
      <Stage>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, marginBottom: 10 }}>DECISION 1 · CONSEQUENCE</div>
        <AVPlaceholder label="Decision 1 consequence narration" text={lines.join("\n\n")} />
        <Narration lines={lines} speakable={false} />
        <PrimaryButton onClick={() => goto("d4_5")}>Continue to decision pause 2</PrimaryButton>
      </Stage>
    );
  }

  // D-4.5 — Decision Pause 2 (post-mortem leadership)
  else if (st.screen === "d4_5") {
    const p2 = D2_CONTENT.segmentD.pause2;
    body = (
      <Stage>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, marginBottom: 10 }}>D · DECISION PAUSE 2 · POST-MORTEM LEADERSHIP</div>
        <AVPlaceholder label="Decision Pause 2 setup" text={p2.narration.join("\n\n")} />
        <Narration lines={p2.narration} speakable={false} />
        <Decision persistKey={st.screen} prompt={p2.prompt} options={p2.options} justificationPrompt={p2.justificationPrompt} minChars={25}
          audioLabel="Decision Pause 2 prompt" audioText={p2.prompt}
          onSubmit={(path, j) => {
            dispatch({ type: "SET_D_PAUSE2", value: { path, justification: j } });
            goto("d4_6");
          }} />
      </Stage>
    );
  ...
  // D-4.6 — Decision 2 consequence
  else if (st.screen === "d4_6") {
    const p2 = D2_CONTENT.segmentD.pause2;
    const chosen = st.segmentDPause2?.path;
    const lines = chosen ? p2.consequences[chosen] : [];
    body = (
      <Stage>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, marginBottom: 10 }}>DECISION 2 · CONSEQUENCE</div>
        <AVPlaceholder label="Decision 2 consequence narration" text={lines.join("\n\n")} />
        <Narration lines={lines} speakable={false} />
        <PrimaryButton onClick={() => goto("d4_7")}>Continue to artifact-write</PrimaryButton>
      </Stage>
    );
  }

  // D-4.7 — Audio case artifact-write (Priya's Slack message + 3 references)
  else if (st.screen === "d4_7") {
    const aw = D2_CONTENT.segmentD.pause2.artifactWrite;
    body = (
      <Stage>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, marginBottom: 14 }}>D-4.7 · AUDIO CASE ARTIFACT-WRITE</div>
        <AVPlaceholder label="Audio case artifact-write prompt" text={aw.prompt} />
        <ArtifactWrite persistKey={st.screen} prompt={aw.prompt} submitLabel={aw.submitLabel}
          references={aw.references} refKind="list" closing={aw.closing}
          minChars={aw.minChars}
          onDone={(t) => { dispatch({ type: "SET_D_PAUSE2_ARTIFACT", text: t }); goto("d5"); }} />
      </Stage>
    );
  }

  // D-5 — Reflection (after the artifact-write per script)
  else if (st.screen === "d5") {
    body = (
      <Stage narrow>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, marginBottom: 14 }}>D-5 · REFLECTION</div>
        <Reflection persistKey={st.screen} prompt={D2_CONTENT.segmentD.reflection.prompt} minChars={30} onDone={(text) => {
          dispatch({ type: "SET_C_RESPONSE", key: "d_reflection", value: text });
          goto("d6");
        }} />
      </Stage>
    );
  ...
  // D-6 — Part 3 / What Happened narration
  else if (st.screen === "d6") {
    const p3 = D2_CONTENT.segmentD.part3;
    body = (
      <Stage>
        <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: C.tealMid, marginBottom: 10 }}>PART 3 · WHAT HAPPENED</div>
        <AVPlaceholder label="Audio Part 3 · What Happened" text={p3.narration.join("\n\n")} />
    ...
    body = (
      <Stage bg={C.navyDeep} narrow>
        <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 14, padding: 26, borderLeft: `3px solid ${C.teal}`, textAlign: "center" }}>
          {D2_CONTENT.segmentD.close.card.map((l, i) => (
  ...
  /* ============== SEGMENT D → E — CLOSE AND PAUSE INVITATION ============== */
    ...
    const dc = D2_CONTENT.segmentD.complete;
    body = (
      <Stage bg={C.navyDeep} narrow>
    ...
    const bp = D2_CONTENT.segmentD.breakPoint2;
    body = (
      <Stage bg={C.navyDeep} narrow>
  ...
  /* ============== SEGMENT E — SCENARIO LAB ============== */
    ...
    body = (
      <Stage bg={C.navyDeep} narrow>
        ...
        <SectorAssignment module={D2_CONTENT.module} />
  ...
  /* ============== SEGMENT F — MICRO-DRILLS ============== */
    ...
    const f1 = D2_CONTENT.segmentF.f1;
    ...
    body = (
      <Stage>
        ...
        <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "14px 16px", marginBottom: 18, fontFamily: SERIF, fontStyle: "italic", fontSize: 16, color: "rgba(255,255,255,0.85)", lineHeight: 1.6 }}>{item.vignette}</div>
        <ArtifactWrite persistKey={st.screen} key={st.screen} prompt={item.prompt} submitLabel="Save my version" references={item.references} refKind="list"
          minChars={20}
          onDone={(text) => {
            const a = localAnalyze(text);
            dispatch({ type: "ANALYZE_ADD", analysis: { ...a, scenario: `F1 · vignette ${idx + 1}` } });
            dispatch({ type: "SET_F_ANSWER", exercise: "f1", value: { idx, text } });
            goto(next);
          }} />
      </Stage>
    );
    ...
    body = (
      <Stage bg={C.navyDeep} narrow>
        <div style={{ background: "rgba(13,148,136,0.1)", border: `1px solid ${C.teal}`, borderRadius: 12, padding: 26, textAlign: "center" }}>
          <p style={{ fontFamily: SERIF, fontStyle: "italic", fontSize: 17, color: C.white, lineHeight: 1.6, margin: 0 }}>{D2_CONTENT.segmentF.f1.closeCard}</p>
        </div>
    ...
    const f2 = D2_CONTENT.segmentF.f2;
    ...
    body = (
      <Stage>
        ...
        <ArtifactWrite persistKey={st.screen} key={st.screen} prompt={item.prompt} submitLabel="Save my version" references={item.references} refKind="list"
          ...
          onDone={(text) => {
            const a = localAnalyze(text);
            dispatch({ type: "ANALYZE_ADD", analysis: { ...a, scenario: `F2 · vignette ${idx + 1}` } });
            dispatch({ type: "SET_F_ANSWER", exercise: "f2", value: { idx, text } });
            goto(next);
          }} />
      </Stage>
    );
    ...
    body = (
      <Stage bg={C.navyDeep} narrow>
        <div style={{ background: "rgba(13,148,136,0.1)", border: `1px solid ${C.teal}`, borderRadius: 12, padding: 26, textAlign: "center" }}>
          <p style={{ fontFamily: SERIF, fontStyle: "italic", fontSize: 17, color: C.white, lineHeight: 1.6, margin: 0 }}>{D2_CONTENT.segmentF.f2.closeCard}</p>
        </div>
    ...
    const fc = D2_CONTENT.segmentF.complete;
    body = (
      <Stage bg={C.navyDeep} narrow>
    ...
    body = (
      <Stage bg={C.paper} narrow>
        <div style={{ textAlign: "center", marginBottom: 18 }}>
          ...
          <div style={{ fontFamily: SERIF, fontSize: 20, color: C.navy }}>Behavioural Dimension No. 2 — Accountability &amp; Ownership</div>
          <div style={{ fontFamily: SERIF, fontStyle: "italic", fontSize: 13.5, color: C.inkSoft, marginTop: 6 }}>{D2_CONTENT.dimension.central_question}</div>
        </div>
  ...
  // participant alone.
  const canBack = st.history.length > 0 && showChrome && st.screen !== "a4" && st.screen !== "g15" && st.screen !== "g4" && st.screen !== "done";
